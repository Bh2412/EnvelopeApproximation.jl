...
Maya_results_df = CSV.read("scripts/benchmarks/comparison_to_Maya/gravitational_potential/Set5.csv", DataFrame)
ηs = Maya_results_df[!, :eta] |> unique
ks = Maya_results_df[!, :k_hor] |> unique
begin
...
begin
    ...
    xlabel!("k_parallel")
    savefig("scripts/benchmarks/comparison_to_Maya/gravitational_potential/Txx_comparison_varying_k.png")
    display(p)
end
...
begin
    ...
    xlabel!("k_parallel")
    savefig("scripts/benchmarks/comparison_to_Maya/gravitational_potential/Tyy_comparison_varying_k.png")
    display(p)
end
...
begin
    ...
    xlabel!("k_parallel") 
    savefig("scripts/benchmarks/comparison_to_Maya/gravitational_potential/Tzz_comparison_varying_k.png")
    display(p)
end
...
begin
    ...
    xlabel!("k_parallel") 
    savefig("scripts/benchmarks/comparison_to_Maya/gravitational_potential/Txz_comparison_varying_k.png")
    display(p)
end
...
@time ŋ = Ŋ(kvecs, current_bubbles(snapshot, η_max), a=a, G=G; rtol=1e-12)
...
begin
    n = 119
    p = plot(ks[1:n], Maya_ψ[:, end][1:n], label="Maya", title="Ψ", xlabel="k_parallel")
    plot!(ks[1:n], _Ψ[1:n] .|> real, label="Ben")
    display(p)
    savefig("scripts/benchmarks/comparison_to_Maya/gravitational_potential/psi_comparison_varying_k.png")
end
...
begin
    n = 119
    p = plot(ks[1:n], Maya_Φ[:, end][1:n], label="Maya", title="Φ", xlabel="k_parallel")
    plot!(ks[1:n], ϕ[1:n] .|> real, label="Ben")
    display(p)
    savefig("scripts/benchmarks/comparison_to_Maya/gravitational_potential/phi_comparison_varying_k.png")
end
